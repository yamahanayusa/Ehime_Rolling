...
{
    // 画像を読み込む
    m_spriteRender.Init("Assets/StageSelect/kurikku.dds", 1920.0f, 1080.0f); //
    m_spriteRender.SetPosition({ 0.0f, 0.0f, 0.0f }); //
    m_spriteRender.SetScale({ 1.0f, 1.0f, 1.0f }); //
    m_spriteRender.Update(); //

    m_spriteRender0.Init("Assets/StageSelect/black.dds", 250.0f, 250.0f); //
    // カーソルの初期位置を設定 (m_button=0, つまりステージ選択なし、または最初のステージ)
    // ここでは、デフォルトでステージ1の位置にカーソルを置くことにします。
    m_spriteRender0.SetPosition({ -400.0f, 100.0f, 0.0f }); // ステージ1の位置
    m_spriteRender0.SetScale({ 1.5f, 1.5f, 1.0f }); //
    m_spriteRender0.Update(); //

    m_spriteRender1.Init("Assets/StageSelect/one.dds", 200.0f, 200.0f); //
    m_spriteRender1.SetPosition({ -400.0f, 100.0f, 0.0f }); //
    m_spriteRender1.SetScale({ 1.5f, 1.5f, 1.0f }); //
    m_spriteRender1.Update(); //

    m_spriteRender2.Init("Assets/StageSelect/Two.dds", 200.0f, 200.0f); //
    m_spriteRender2.SetPosition({ 0.0f, 100.0f, 0.0f }); //
    m_spriteRender2.SetScale({ 1.5f, 1.5f, 1.0f }); //
    m_spriteRender2.Update(); //

    m_spriteRender3.Init("Assets/StageSelect/Three.dds", 200.0f, 200.0f); //
    m_spriteRender3.SetPosition({ 400.0f, 100.0f, 0.0f }); //
    m_spriteRender3.SetScale({ 1.5f, 1.5f, 1.0f }); //
    m_spriteRender3.Update(); //

    m_spriteRender4.Init("Assets/StageSelect/Four.dds", 200.0f, 200.0f); //
    m_spriteRender4.SetPosition({ -200.0f, -300.0f, 0.0f }); //
    m_spriteRender4.SetScale({ 1.5f, 1.5f, 1.0f }); //
    m_spriteRender4.Update(); //

    m_spriteRender5.Init("Assets/StageSelect/Five.dds", 200.0f, 200.0f); //
    m_spriteRender5.SetPosition({ 200.0f, -300.0f, 0.0f }); //
...
void StageSelect::Update()
{
    if (g_pad[0]->IsTrigger(enButtonRight))
    {
        ...
        {
        case 1:
            m_spriteRender0.SetPosition({ -400.0f, 100.0f, 0.0f }); //
            break;
        case 2:
        ...
        case 3:
            ...
            break;
        case 4:
            m_spriteRender0.SetPosition({ -200.0f, -300.0f, 0.0f }); //
            break;
        case 5:
            m_spriteRender0.SetPosition({ 200.0f, -300.0f, 0.0f }); //
            break;
